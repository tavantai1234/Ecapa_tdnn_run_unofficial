#!/usr/bin/env python3
"""Fine-tune pretrained ECAPA-TDNN on macOS using Apple MPS."""
...
def use_mps_by_default():
    """Add --device=mps unless the user explicitly chooses a device."""
    if not any(arg.startswith("--device") for arg in sys.argv[1:]):
        sys.argv.append("--device=mps")
...
def validate_device(device):
    if str(device).startswith("mps"):
        ...
        if not torch.backends.mps.is_available():
            raise RuntimeError(
                "MPS is unavailable. Check that the Mac uses Apple Silicon "
                "or a supported AMD GPU and that macOS/PyTorch are current."
            )
